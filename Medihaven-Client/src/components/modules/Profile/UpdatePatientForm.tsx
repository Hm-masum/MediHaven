...
export default function UpdatePatientForm({
  ...
  const [imagePreview, setImagePreview] = useState(patientData.profilePhoto);
  ...
  const form = useForm({
    defaultValues: {
      ...
      patientHealthData: {
        ...
        dietaryPreferences:patientData?.patientHealthData?.dietaryPreferences || "",
        mentalHealthHistory:patientData?.patientHealthData?.mentalHealthHistory || "",
        immunizationStatus:patientData?.patientHealthData?.immunizationStatus || "",
        hasPastSurgeries:patientData?.patientHealthData?.hasPastSurgeries ?? false,
        ...
        pregnancyStatus:patientData?.patientHealthData?.pregnancyStatus ?? false,
        recentAnxiety: patientData?.patientHealthData?.recentAnxiety ?? false,
        recentDepression:patientData?.patientHealthData?.recentDepression ?? false,
      },
    },
  });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            ...
            {[
              { name: "patientHealthData.hasAllergies", label: "Has Allergies" },
              { name: "patientHealthData.hasDiabetes", label: "Has Diabetes" },
              { name: "patientHealthData.smokingStatus", label: "Smoking Status" },
              { name: "patientHealthData.pregnancyStatus", label: "Pregnancy Status" },
              { name: "patientHealthData.hasPastSurgeries", label: "Past Surgeries" },
              { name: "patientHealthData.recentAnxiety", label: "Recent Anxiety" },
              { name: "patientHealthData.recentDepression", label: "Recent Depression" },
            ].map((item) => (
              <FormField
                key={item.name}
                control={form.control}
